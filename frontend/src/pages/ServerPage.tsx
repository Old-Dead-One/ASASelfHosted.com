...
function CopyableInternalLink({
    ...
    copyValue: string
    label: string
    /** Text to show inside the link (can be truncated) */
    displayValue: string
    className?: string
}) {
    const [copied, setCopied] = useState(false)
        ...
        <div className={className}>
            <div className="flex items-center justify-between gap-2">
                ...
                <button
                    type="button"
                    onClick={copy}
                    className="text-xs text-primary hover:text-accent transition-colors focus:outline-none focus:ring-2 focus:ring-ring rounded px-1 min-h-[32px] whitespace-nowrap"
                    aria-label={`Copy ${label.toLowerCase()}`}
                    aria-live="polite"
                >
                    {copied ? 'Copied!' : 'Copy'}
            ...
            <dd className="text-sm text-foreground font-mono bg-muted p-1.5 rounded">
                <Link
                    to={to}
                    className="block truncate text-primary hover:text-accent transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded px-0.5"
                    title={copyValue}
                >
                    {displayValue}
                </Link>
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative overflow-hidden rounded-lg tek-border">
                        ...
                        <div className={`relative ${CARD_PADDING} bg-background/65 backdrop-blur-[1px] flex flex-col min-h-0 max-h-[560px]`}>
                            <div className="flex items-start justify-between gap-3 mb-2">
                                <h2 className="text-base font-semibold text-foreground">Server Details</h2>
                                <div className="text-xs text-muted-foreground whitespace-nowrap shrink-0">
                                    Created <span className="text-foreground">{new Date(server.created_at).toLocaleDateString()}</span>
                                </div>
                            </div>
                            <div className="flex-1 min-h-0 overflow-hidden">
                                <div className="grid grid-cols-1 md:grid-cols-[1fr_1.5fr] gap-4 h-full min-h-0">
                                    <div>
                                        <dl className="space-y-1">
                                            ...
                                            {server.quality_score !== null ? (
                                                ...
                                                <>
                                                    <dt className="text-sm font-medium text-muted-foreground">Quality Score</dt>
                                                    <dd className="text-sm text-muted-foreground italic">Not available</dd>
                                                </>
                                            )}
                                            {server.rates ? (
                            ...
                            <div className="shrink-0 mt-1.5">
                                <h3 className="text-sm font-medium text-muted-foreground mb-1">Wipe Info</h3>
                                {server.wipe_info ? (
                                    <p className="text-sm text-foreground whitespace-pre-line">{server.wipe_info}</p>
                                ) : (
                                    ...
                                    {server.cluster_slug ? (() => {
                                        const fullUrl =
                                            typeof window !== 'undefined'
                                                ? `${window.location.origin}/clusters/${server.cluster_slug}`
                                                : `/clusters/${server.cluster_slug}`
                                        const display = fullUrl.replace(/^https?:\/\//, '')
                                        return (
                                            <CopyableInternalLink
                                                label="Cluster link"
                                                to={`/clusters/${server.cluster_slug}`}
                                                copyValue={fullUrl}
                                        ...
                                        <>
                                            <dt className="text-sm font-medium text-muted-foreground">Cluster link</dt>
                                            <dd className="text-sm text-muted-foreground italic">Not specified</dd>
                                        </>
                                    )}
