...
class TestDirectoryResponseShape:
    ...
    async def test_list_servers_response_shape(
        ...
        for server in servers:
            ...
            assert server.seconds_since_seen is None or isinstance(
                server.seconds_since_seen, (int, float)
            )
...
class TestCursorPagination:
    ...
    async def test_cursor_encoding_decoding(self, directory_repo: DirectoryRepository):
        ...
        servers1, next_cursor = await directory_repo.list_servers(
            limit=3, rank_by="updated", order="desc"
        )
    ...
    async def test_cursor_pagination_no_duplicates(
        ...
        for _ in range(5):  # Max 5 pages to avoid infinite loop
            servers, next_cursor = await directory_repo.list_servers(
                limit=3, cursor=cursor, rank_by=rank_by, order=order
            )
    ...
    async def test_cursor_mismatch_rejection(self, directory_repo: DirectoryRepository):
        ...
        with pytest.raises(
            DomainValidationError, match="Cursor sort_by/order mismatch"
        ):
    ...
    async def test_cursor_pagination_determinism(
        self, directory_repo: DirectoryRepository
    ):
        ...
        servers2a, _ = await directory_repo.list_servers(
            limit=3, cursor=next_cursor, rank_by="updated", order="desc"
        )
...
class TestSorting:
    ...
    @pytest.mark.parametrize(
        "rank_by", ["updated", "new", "favorites", "players", "quality", "uptime"]
    )
    @pytest.mark.parametrize("order", ["asc", "desc"])
    async def test_sorting_order(
        self, directory_repo: DirectoryRepository, rank_by: RankBy, order: SortOrder
    ):
        """Test that sorting works correctly for each rank_by and order."""
        servers, _ = await directory_repo.list_servers(
            limit=10, rank_by=rank_by, order=order
        )
        ...
        non_null_pairs = [
            ...
            if get_sort_value(servers[i]) is not None
            and get_sort_value(servers[i + 1]) is not None
        ]

        if not non_null_pairs:
            pytest.skip(
                f"No non-null values for rank_by={rank_by}, cannot verify ordering"
            )
        ...
        for i, val1, val2 in non_null_pairs:
            if order == "desc":
                assert val1 >= val2 or (
                    val1 == val2 and servers[i].id < servers[i + 1].id
    ...
    async def test_tie_break_ordering(self, directory_repo: DirectoryRepository):
        ...
        servers, _ = await directory_repo.list_servers(
            limit=10, rank_by="updated", order="desc"
        )
...
class TestFiltering:
    ...
    async def test_status_filter(self, directory_repo: DirectoryRepository):
        ...
        for status in ["online", "offline", "unknown"]:
            servers, _ = await directory_repo.list_servers(limit=100, status=status)
    ...
    async def test_ruleset_filter(self, directory_repo: DirectoryRepository):
        """Test ruleset filter."""
        servers, _ = await directory_repo.list_servers(limit=100, ruleset="vanilla")
    ...
    async def test_game_mode_filter(self, directory_repo: DirectoryRepository):
        ...
        servers, _ = await directory_repo.list_servers(limit=100, game_mode="pve")
    ...
    async def test_cluster_visibility_filter(self, directory_repo: DirectoryRepository):
        ...
        servers, _ = await directory_repo.list_servers(
            limit=100, cluster_visibility="public"
        )

        for server in servers:
            assert (
                server.cluster_visibility == "public"
                or server.cluster_visibility is None
            )

    async def test_players_range_filter(self, directory_repo: DirectoryRepository):
        """Test players_current_min/max filters."""
        servers, _ = await directory_repo.list_servers(
            limit=100, players_current_min=10, players_current_max=50
        )
    ...
    async def test_quality_min_filter(self, directory_repo: DirectoryRepository):
        ...
        servers, _ = await directory_repo.list_servers(limit=100, quality_min=80.0)
    ...
    async def test_uptime_min_filter(self, directory_repo: DirectoryRepository):
        ...
        servers, _ = await directory_repo.list_servers(limit=100, uptime_min=90.0)
...
class TestSecondsSinceSeen:
    ...
    async def test_seconds_since_seen_consistency(
        self, directory_repo: DirectoryRepository
    ):
        ...
        seen_values = [
            s.seconds_since_seen for s in servers if s.seconds_since_seen is not None
        ]
    ...
    async def test_seconds_since_seen_null_when_last_seen_null(
        self, directory_repo: DirectoryRepository
    ):
    ...
    async def test_seconds_since_seen_non_negative(
        self, directory_repo: DirectoryRepository
    ):
...
class TestClusterVisibility:
    ...
    async def test_list_clusters_public_only(
        self, clusters_repo: DirectoryClustersRepository
    ):
        """Test that list_clusters returns only public clusters."""
        clusters, _ = await clusters_repo.list_clusters(limit=100, visibility="public")

        for cluster in clusters:
            assert cluster.visibility == "public"

    async def test_list_clusters_rejects_unlisted(
        self, clusters_repo: DirectoryClustersRepository
    ):
        """Test that list_clusters rejects unlisted visibility filter."""
        with pytest.raises(
            DomainValidationError, match="Unlisted clusters are not accessible"
        ):
            await clusters_repo.list_clusters(limit=100, visibility="unlisted")

    async def test_get_cluster_public_returns_200(
        self, clusters_repo: DirectoryClustersRepository
    ):
    ...
    async def test_get_cluster_unlisted_returns_none(
        self, clusters_repo: DirectoryClustersRepository
    ):
...
class TestSearch:
    ...
    async def test_search_by_name(self, directory_repo: DirectoryRepository):
        ...
        servers, _ = await directory_repo.list_servers(limit=100, q="Sun Bros")

        # All results should match search query
        for server in servers:
            assert (
                "sun" in server.name.lower()
                or "bros" in server.name.lower()
                or (
                    server.description
                    and (
                        "sun" in server.description.lower()
                        or "bros" in server.description.lower()
                    )
                )
            )
    ...
    async def test_search_no_results(self, directory_repo: DirectoryRepository):
        ...
        servers, _ = await directory_repo.list_servers(
            limit=100, q="nonexistent_server_name_xyz123"
        )

        assert len(servers) == 0

    async def test_search_cursor_incompatibility(
        self, directory_repo: DirectoryRepository
    ):
...
class TestDefaultSort:
    ...
    async def test_default_sort_explicit(self, directory_repo: DirectoryRepository):
        ...
        servers2, _ = await directory_repo.list_servers(
            limit=10, rank_by="updated", order="desc"
        )
