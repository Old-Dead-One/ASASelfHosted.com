...
class SupabaseHeartbeatJobsRepository(HeartbeatJobsRepository):
    ...
    async def claim_jobs(self, batch_size: int) -> list[HeartbeatJob]:
        ...
        try:
            ...
            try:
                self._supabase.table("heartbeat_jobs").update({"claimed_at": None}).is_(
                    "processed_at", "null"
                ).lt("claimed_at", cutoff_iso).execute()
            except Exception:
            ...
            for job_data in jobs_data:
                ...
                enqueued_at = datetime.fromisoformat(
                    job_data["enqueued_at"].replace("Z", "+00:00")
                )
                processed_at = None
                if job_data.get("processed_at"):
                    processed_at = datetime.fromisoformat(
                        job_data["processed_at"].replace("Z", "+00:00")
                    )
    ...
    async def mark_processed(self, job_id: str, processed_at: datetime) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to mark job {job_id} as processed: {str(e)}"
            ) from e
    ...
    async def mark_failed(self, job_id: str, error: str, attempts: int) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to mark job {job_id} as failed: {str(e)}"
            ) from e
