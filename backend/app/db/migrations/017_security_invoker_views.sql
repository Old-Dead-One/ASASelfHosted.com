-- ASASelfHosted.com - Fix Supabase SECURITY DEFINER warnings
-- Recreates directory_view and heartbeats_public with SECURITY INVOKER so that
-- RLS and permissions of the querying user apply (not the view owner).
-- Run after 012_security_fixes.sql. Works after 013, 014, or 015 (checks for
-- servers.rulesets and servers.join_password and builds the view accordingly).

-- ============================================================================
-- directory_view: SECURITY INVOKER
-- ============================================================================
-- Build view definition based on which columns exist on servers (013 vs 014 vs 015).
...
BEGIN
    ...
    IF has_rulesets AND has_join_password THEN
        -- After 015: full definition with rulesets and join_password
        EXECUTE '
    ...
    ELSIF has_join_password THEN
        -- After 014: join_password on servers, no rulesets column yet (derive rulesets from ruleset)
        EXECUTE '
        CREATE VIEW directory_view WITH (security_invoker = true) AS
        SELECT
            s.id, s.name, s.description, s.map_name, s.join_address, s.join_password,
        ...
        WHERE s.hosting_provider = ''self_hosted''';
    ELSE
        -- After 013: no join_password on servers (012 moved it to server_secrets), no rulesets (derive from ruleset)
        EXECUTE '
        CREATE VIEW directory_view WITH (security_invoker = true) AS
        SELECT
...
-- ============================================================================
-- heartbeats_public: SECURITY INVOKER
-- ============================================================================
-- Same definition as 012; only the view options change.
...
BEGIN
    IF EXISTS (
        ...
        EXECUTE '
        CREATE VIEW heartbeats_public WITH (security_invoker = true) AS
        SELECT
            id,
            server_id,
            source,
