-- Sprint 8: Exclude hidden servers from directory_view.
-- Run after 025. Adds AND (s.hidden_at IS NULL) to directory_view WHERE clause.
...
BEGIN
    ...
    IF has_rulesets AND has_join_password THEN
        ...
        WHERE s.hosting_provider = ''self_hosted'' AND (s.hidden_at IS NULL)';
...
COMMENT ON VIEW directory_view IS 'Public read model. Excludes hidden servers (hidden_at IS NULL). SECURITY INVOKER.';
