...
def detect_player_spike_anomaly(
    ...
    for i in range(len(heartbeats) - 2):
        ...
        time_delta_oldest_to_middle = (
            received_middle - received_oldest
        ).total_seconds()
        time_delta_middle_to_newest = (
            received_newest - received_middle
        ).total_seconds()
        ...
        if (
            players_oldest == 0
            and players_middle >= 50
            and players_newest == 0
            and total_time < 60
        ):
        ...
        if players_middle > 0 and players_newest > players_middle:
            ...
            capacity = (
                hb_newest.get("players_capacity") or 70
        ...
        if (
            players_middle >= 30
            and players_newest == 0
            and time_delta_middle_to_newest < 10
        ):
            anomaly_detected = True
            latest_anomaly_time = received_newest
            break
