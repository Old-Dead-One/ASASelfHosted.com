...
def compute_quality_score(
    ...
    if players_current is not None and players_capacity is not None and players_capacity > 0:
        ...
        fill_rate = min(1.0, max(0.0, players_current / players_capacity))  # Clamp to 0-1
        activity_component = ACTIVITY_WEIGHT * (fill_rate * 100.0)  # Scale to 0-100, then apply weight
    ...
    confidence_multiplier = CONFIDENCE_MULTIPLIERS.get(confidence, 0.3)  # Default to red if unknown
